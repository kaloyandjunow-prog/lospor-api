...
export const DATA_DICTIONARY: DictionaryEntry[] = [
  ...
  },
  {
    ...
    meaning: "Anaesthesia techniques used (multi-select), one procedure row per "
      ...
      + "blocks inherit something true. A general anaesthetic is coded by "
      + "what maintained it -- 4118897 (Inhalation general anesthesia) or "
      + "4086418 (Total intravenous anesthesia). A balanced anaesthetic stays "
      + "at the parent 4171773 on purpose: TIVA means total intravenous, so a "
      + "case running a volatile is not TIVA, and it is not inhalation-only "
      + "either, so neither sibling is true of it and the parent is the correct "
      + "answer rather than a fallback. The drug rows distinguish maintenance "
      ...
      + "not an omission. REGIONAL itself is deliberately not "
